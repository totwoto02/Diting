...
class EntropyManager:
    ...
    def calculate_entropy(self, slice_id: str) -> Dict:
        ...
        if old_entropy is not None:
            self._log_change(
                slice_id, old_entropy, entropy, old_level, new_level, "自动熵计算", "system"
            )
        ...
        return {
            ...
            "new_level": new_level,
            "trend": trend,
            "factors": {"option_count": option_count, "version": version, "days_open": days_open},
        }
    ...
    def get_project_entropy(self, project_path: str) -> Dict:
        ...
        if not rows:
            return {"avg_entropy": 0, "level": "unknown", "memory_count": 0}
    ...
    def detect_entropy_anomaly(self, slice_id: str) -> Dict:
        ...
        if level == "low" and trend == "increasing":
            anomalies.append(
                {"type": "decision_reversed", "message": "已决策方案熵值升高，可能有人翻旧账"}
            )

        # 异常 2: 执行中方案熵值升高（可能执行受阻）
        if "执行" in memory.get("content", "") and entropy > 50:
            anomalies.append(
                {"type": "execution_blocked", "message": "执行中方案熵值升高，可能执行受阻"}
            )
        ...
        if entropy >= 70 and temp == "cold":
            anomalies.append(
                {"type": "chaotic_forgotten", "message": "高熵低温：混乱但被遗忘，幻觉高危"}
            )
