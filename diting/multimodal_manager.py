...
class MultimodalMemoryManager:
    ...
    def store_image(
        ...
        should_call_ai = self._should_call_ai(
            file_type="image", file_data=image_data, force_flag=generate_ai_summary
        )
        ...
        """,
            (
                ...
                original_filename,
                ai_summary,
                json.dumps(ai_keywords),
                json.dumps(ai_entities),
                ai_confidence,
                "warm",
                50,
    ...
    def _save_file(self, file_data: bytes, file_type: str, original_filename: str = None) -> str:
        """保存文件到存储目录"""
        # 按日期组织目录
        ...
        extension = (
            ...
            else ("jpg" if file_type == "image" else "ogg")
        )
