...
class IntegrityTracker:
    ...
    def _init_schema(self):
        ...
        # 创建索引
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_log_path ON integrity_log(v_path)")
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_log_time ON integrity_log(timestamp)")
    ...
    def track_update(
        ...
        self.conn.execute(
            ...
            VALUES (?, 'UPDATE', ?, ?, ?, ?, ?, ?)
        """,
            (v_path, old_hash, new_hash, old_content[:1000], new_content[:1000], reason, operator),
        )
    ...
    def track_create(self, v_path: str, content: str, operator: str = "AI") -> Dict[str, Any]:
        """
        追踪创建操作
    ...
    def track_delete(self, v_path: str, old_content: str, reason: str = "") -> Dict[str, Any]:
        """
        追踪删除操作
        ...
        # 从哈希表移除
        self.conn.execute("DELETE FROM content_hashes WHERE v_path = ?", (v_path,))
    ...
    def verify_integrity(self, v_path: str, current_content: str) -> Dict[str, Any]:
        ...
        if not row:
            return {"verified": False, "reason": "未找到哈希记录", "is_tampered": None}
    ...
    def get_stats(self) -> Dict[str, Any]:
        """获取统计信息"""
        cursor = self.conn.execute("SELECT COUNT(*) FROM integrity_log")
        total_logs = cursor.fetchone()[0]

        cursor = self.conn.execute("SELECT COUNT(*) FROM content_hashes")
        ...
        return {"total_logs": total_logs, "tracked_files": tracked_files, "by_action": by_action}
