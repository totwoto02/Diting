...
class SmartTrigger:
    ...
    def _check_size_threshold(self, file_type: str, file_size: int) -> bool:
        ...
        return thresholds["min_call"] <= file_size <= thresholds["max_call"]
    ...
    def _analyze_memory_path(self, memory_path: str) -> float:
        ...
        # 低分路径
        low_score_paths = ["/temp", "/cache", "/test", "/trash", "/临时", "/缓存", "/测试", "/垃圾"]
...
if __name__ == "__main__":
    # 创建触发器
    trigger = SmartTrigger({"ENABLE_SMART_TRIGGER": True, "AI_MONTHLY_QUOTA": 100})
